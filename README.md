...
```bash
# pip가 없으면 먼저 설치
sudo apt install -y python3-pip    # Ubuntu/WSL
...
> **WSL 참고**: conda 환경에 설치한 경우 `conda activate base`를 먼저 실행해야 한다. 매번 활성화가 번거로우면 시스템 Python에 설치하는 것을 권장한다.
...
새 주제의 RAG를 추가하려면 3단계로 진행한다:

```bash
# 1. 워크스페이스 등록 (디렉토리 없으면 자동으로 init까지 수행)
lab-memory workspace add metabolic ~/metabolic-eng

# 2. 데이터 넣기 (raw 폴더에 PPT/PDF 복사 또는 심볼릭 링크)
